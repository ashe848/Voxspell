...
public class DataHandler {
	 ...
	 * @author Abby S
	 */
	private static void readSpellingListName() {
		try {
			File apelling_list_name = new File(parent_frame.getResourceFileLocation()+user+"_spellinglistname");
			if (!apelling_list_name.exists()) {
				apelling_list_name.createNewFile();	
				spelling_list_name="NZCER-spelling-lists.txt";
			} else {
				BufferedReader current_BR = new BufferedReader(new FileReader(apelling_list_name));
				spelling_list_name = current_BR.readLine();	
				current_BR.close();
			}
...
private static void setupFiles() {
	...
	reviewlist = parent_frame.getResourceFileLocation()+user+"_"+spelling_list_name+"_reviewlist";
	statsfile = parent_frame.getResourceFileLocation()+user+"_"+spelling_list_name+"_statsfile";
	settings = parent_frame.getResourceFileLocation()+user+"_settings";
...
public static void writeToSettingsFiles(){
	try {
		FileWriter fw_settings = new FileWriter(new File(settings), false);
		fw_settings.write(current_level+" "+parent_frame.getFestival().getFestivalSpeed().getSpeedValue()+" "+parent_frame.getFestival().getFestivalVoice().getVoiceValue()+" "+words_in_quiz);
		fw_settings.close();
		
		FileWriter fw_spellinglistname = new FileWriter(new File(parent_frame.getResourceFileLocation()+user+"_spellinglistname"), false);
		fw_spellinglistname.write(spelling_list_name);
		fw_spellinglistname.close();
	} catch (IOException e) {
		e.printStackTrace();
	}
}
